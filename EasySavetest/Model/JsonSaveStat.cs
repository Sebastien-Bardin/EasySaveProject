...
namespace EasySavetest.Model
{
    class SaveStat
    {
        ...
        public int time = 0;
        public TimeSpan CryptTime;

        public void RefreshSaveStat(SaveStat saveStat)
        {
            //Creating Json object
            LogPattern save = new LogPattern() { LastUpdate = DateTime.Now.ToString("dd/mm/yy HH:mm"), FilesNumber = saveStat.filecount, FilesSize = saveStat.totalsize, FilesRemaining = saveStat.remainingfiles, SizeRemaining = saveStat.remainingsize, LastWork = saveStat.currentfiletocopy, Progression = 100 - ((saveStat.remainingsize + 1 / (saveStat.totalsize)) * 100) + "%" , Duration = saveStat.time};
